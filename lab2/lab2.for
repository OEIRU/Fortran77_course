...
109       CONTINUE
      ...
      REAL*8 FUNCTION ARCCOS(SUM)
          ...
          IF (SUM .LT. -1.0D0 .OR. SUM .GT. 1.0D0) THEN
              ARCCOS = -1.0D0
          ELSE IF (ABS(SUM - 1.0D0) .LT. EPSILON) THEN
              ARCCOS = 0.0D0
          ELSE
              ARCCOS = ACOS(SUM)
          END IF
      ...
      SUBROUTINE WRITE_ROW(X, M, REAL_M)
          ...
          DO J = 1, M - 2, 1
              ...
              END IF
              Y = Y + Y_STEP
              IF (ABS(Y) .LT. ABS(Y_STEP / 2.0D0)) THEN
                  Y = 0.0D0
              END IF
              ARCCOS_RES = ARCCOS(X + Y)
              IF (ARCCOS_RES .GE. 0.0D0) THEN
...
109       CONTINUE
      ...
      SUBROUTINE CREATE_TABLE()
          IMPLICIT NONE
          COMMON /INPUT/ X_MAX, Y_MAX, X_MIN, Y_MIN, X_STEP, Y_STEP
          REAL*8 X_MAX, Y_MAX, X_MIN, Y_MIN, X_STEP, Y_STEP, X, Y
          INTEGER N, M, I, J, REAL_M, IS_INVISIBLE
          REAL_M = 1
          N = INT((X_MAX - X_MIN) / X_STEP + 0.5D0)
          IF (X_MIN + (N - 1) * X_STEP .LT. X_MAX) THEN
              N = N + 1
          END IF
          M = INT((Y_MAX - Y_MIN) / Y_STEP + 0.5D0)
          ...
          DO I = 1, N, 1
              ...
              IF (IS_INVISIBLE(X, X + X_STEP) .EQ. 0) THEN
                  IF (ABS(X) .LT. ABS(X_STEP / 2.0D0)) THEN
                      X = 0.0D0
                  END IF
